"""
plot_results_s5010.py  —  Visualize Run 1 output vs. S5010 baseline.

Compares seed_airfoil.dat (preliminary optimized, Run 1 result) against the
original S5010_seed.dat, runs XFOIL on the S5010 if its polar is missing,
then produces a 6-panel figure showing the first-stage improvement.

Usage:
    python plot_results_s5010.py
"""
...
OPT_DAT      = "seed_airfoil.dat"
SEED_DAT     = "S5010_seed.dat"               # original S5010 baseline
OPT_POLAR    = "seed_polar_plot.txt"          # polar for preliminary optimized (Run 1)
SEED_POLAR   = "S5010_seed_polar.txt"         # generated here if missing
...
def main():
    # --- Ensure S5010 polar exists ------------------------------------------
    if not os.path.exists(SEED_POLAR):
        if os.path.exists(SEED_DAT):
            print(f"Running XFOIL on S5010 baseline ({SEED_DAT}) ...")
            run_xfoil(SEED_DAT, SEED_POLAR)
        else:
            print(f"WARNING: {SEED_DAT} not found — S5010 polar will be skipped.")
    ...
    # --- Print summary ------------------------------------------------------
    print("\n" + "=" * 52)
    print(f"{'Metric':<30} {'Prelim. Opt (Run 1)':>19}  {'S5010':>10}")
    print("=" * 52)
    rows = [
        ("χ  (objective)",        "chi",      "{:.1f}"),
    ...
    fig.suptitle(
        ...
        "Stage 1: Prelim. Optimized (Run 1) vs. S5010 Original Baseline",
    ...
    C_SEED = "#ff7f0e"   # orange — S5010 baseline
    ...
    if x_opt is not None:
        ax0.plot(x_opt, y_opt, color=C_OPT,  lw=1.8, label="Prelim. Optimized (Run 1)")
    if x_seed is not None:
        ax0.plot(x_seed, y_seed, color=C_SEED, lw=1.4, ls="--", label="S5010 (original baseline)")
    ...
    ax0.set_xlabel("x/c")
    ax0.set_ylabel("y/c")
    ax0.set_title("Airfoil Geometry Comparison  —  Prelim. Optimized (Run 1) vs. S5010 Baseline")
    ...
    if have_seed_polar:
        ax1.plot(a_seed, cl_seed, color=C_SEED, lw=1.4, ls="--", label="S5010")
    ax1.plot(a_opt, cl_opt, color=C_OPT, lw=1.8, label="Prelim. Optimized (Run 1)")
    ...
    if have_seed_polar:
        ax2.plot(cd_seed, cl_seed, color=C_SEED, lw=1.4, ls="--", label="S5010")
    ax2.plot(cd_opt, cl_opt, color=C_OPT, lw=1.8, label="Prelim. Optimized (Run 1)")
    ...
    if have_seed_polar:
        ...
        ax3.plot(cl_seed, cl_cd_seed, color=C_SEED, lw=1.4, ls="--", label="S5010")
    ax3.plot(cl_opt, cl_cd_opt, color=C_OPT, lw=1.8, label="Prelim. Optimized (Run 1)")
    ...
    if have_seed_polar:
        ax4.plot(cl_seed, cm_seed, color=C_SEED, lw=1.4, ls="--", label="S5010")
    ax4.plot(cl_opt, cm_opt, color=C_OPT, lw=1.8, label="Prelim. Optimized (Run 1)")
    ...
    if have_seed_polar:
        ax5.plot(a_seed, cd_seed, color=C_SEED, lw=1.4, ls="--", label="S5010")
    ax5.plot(a_opt, cd_opt, color=C_OPT, lw=1.8, label="Prelim. Optimized (Run 1)")
    ax5.set_xlabel("α (°)")
    ax5.set_ylabel("Cd")
    ...
    y_pos = 0.97
    dy    = 0.082
    ax6.text(0.5, y_pos + 0.015, "Prelim. Optimized (Run 1) — Summary", ha="center", va="top", fontsize=9.5, fontweight="bold", transform=ax6.transAxes)
    ...
    # Save and show
    # -------------------------------------------------------------------------
    out_png = "airfoil_results_run1_vs_s5010.png"
